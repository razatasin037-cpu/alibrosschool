...
function getCustomerCalculation($customer)
{
    ...
    if (
        ...
        return [
            'current' => 0,
            'remaining' => (int)($customer['total_installments'] ?? 0),
            'progress' => 0,
            'next_payment' => null,
            'status' => 'Active'
        ];
    ...
    $total = (int)($customer['total_installments'] ?? 0);
    ...
    if ($plan === 'Monthly') {
        ...
        if ($today >= $opening) {
            ...
            $openingYear = (int)$opening->format('Y');
            $openingMonth = (int)$opening->format('m');
            ...
            $monthsPassed =
                (($todayYear - $openingYear) * 12)
                +
                ($todayMonth - $openingMonth);

            if ($todayDay >= $paymentDay) {
                $current = $monthsPassed + 1;
            } else {
        ...
        if ($current < $total) {
            ...
            $nextMonthIndex =
                ($openingMonth - 1) + $current;

            $nextYear =
                $openingYear +
                intdiv($nextMonthIndex, 12);
        ...
        $firstMonday = clone $opening;

        $openingWeekDay =
            (int)$firstMonday->format('N');
        ...
        if ($today < $firstMonday) {
            ...
            $daysPassed =
                $firstMonday->diff($today)->days;

            $current =
                floor($daysPassed / 7) + 1;
