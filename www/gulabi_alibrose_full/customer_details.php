...
if ($plan === 'Monthly') {
    ...
    if ($currentPeriod > $totalInstallments) {
        $currentPeriod = $totalInstallments;
    }
    ...
    $openingTimestamp =
        strtotime($openingDate);

    $todayTimestamp =
        strtotime(date('Y-m-d'));

    if ($todayTimestamp < $openingTimestamp) {
...
$paymentStmt = $conn->prepare("
    ...
    WHERE customer_id = ?
    ORDER BY installment_no ASC, payment_date ASC
");
