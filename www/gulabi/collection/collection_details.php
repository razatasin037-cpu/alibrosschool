...
if (isset($_GET['paid']) && $_GET['paid'] === '1') {
    $success = 'Payment successfully receive ho gaya.';
}

if (isset($_GET['absent']) && $_GET['absent'] === '1') {
    $success = 'Absent mark ho gaya. Plan mein 1 extra installment add kar di gayi.';
}
...
function getSchedule(
    ...
    $paid = getPaymentRecords($conn, $customerId);

    $due = [];
    $future = [];

    $totalInstallments = max(0, (int)$totalInstallments);
    ...
    /*
     * Sequential rule:
     * first unpaid installment is the current installment.
     * A future installment is never shown as completed.
     */
    $currentNo = 0;
...
if (isset($_POST['mark_absent'])) {
    ...
    $customerId = (int)($_POST['absent_customer_id'] ?? 0);
    ...
    if ($customerId <= 0) {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                SELECT id, opening_date, plan, total_installments, status
                FROM customers
                WHERE id = ? AND plan = ? AND status = 'Active'
                LIMIT 1
            ");
            $stmt->bind_param('is', $customerId, $plan);
            $stmt->execute();
            $customer = $stmt->get_result()->fetch_assoc();
            $stmt->close();

            if (!$customer) {
                throw new Exception('Customer nahi mila ya active nahi hai.');
            }
            ...
            if (empty($schedule['due'])) {
                throw new Exception('Is customer ka current installment abhi Due nahi hai.');
            }

            $newTotal = (int)$customer['total_installments'] + 1;
            $currentDue = $schedule['due'][0]['due_date'];

            $update = $conn->prepare("
                UPDATE customers
                SET
                    total_installments = ?,
                    absent_installments = absent_installments + 1,
                    next_due_date = ?,
                    status = 'Active'
                WHERE id = ?
            ");
            $update->bind_param('isi', $newTotal, $currentDue, $customerId);
            ...
            header(
                'Location: collection_details.php?plan=' .
                urlencode($plan) .
                '&absent=1'
            );
            exit;
...
<div class="cardx p-3">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div>
            ...
            <div class="muted small">
                Current installment pehle pay hoga. Payment ke baad hi next week/month unlock hoga.
            </div>
                    ...
                    <td>
                        ...
                        <?php if ($hasDue): ?>
                            <form method="post" class="mt-2">
                                <input type="hidden" name="absent_customer_id" value="<?= (int)$customer['id'] ?>">
                                <button
                                    type="submit"
                                    name="mark_absent"
                                    class="btn btn-sm btn-outline-danger"
                                    onclick="return confirm('Is customer ko absent mark karein? Iske plan mein 1 extra installment add hogi.')"
                                >
    ...
    ?.addEventListener(
        ...
        function(event)
        {
            ...
            if (count === 0)
            {
                ...
                alert(
                    'Pehle current due customer select karo.'
                );
            ...
            if (
                !confirm(
                    count +
                    ' customer ka current installment receive karna hai?'
                )
            )
            {
